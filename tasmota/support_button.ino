...
#endif // ESP8266
#ifdef ESP32
      pinMode(Pin(GPIO_KEY1, i), bitRead(Button.pulldown_mask, i) ? INPUT_PULLDOWN : bitRead(Button.no_pullup_mask, i) ? INPUT
                                                                                                                       : INPUT_PULLUP);
#endif // ESP32
    }
#ifdef USE_ADC
...
uint8_t ButtonSerial(uint8_t serial_in_byte)
{
  ...
  { // 0xA0 - Start of Sonoff dual button code
    serial_in_byte = 0;
    Button.dual_code = 0;
  ...
  for (uint32_t button_index = 0; button_index < MAX_KEYS; button_index++)
  {
    ...
    if (button_present)
    {
      ...
      else if (SONOFF_4CHPRO == TasmotaGlobal.module_type)
      {
        ...
        if (button_pressed)
        {
          ...
          { // SetOption73 (0) - Decouple button from relay and send just mqtt topic
            ...
            {                                                                  // Execute Toggle command via MQTT if ButtonTopic is set
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              { // SetOption1 (0) - Control button multipress
        ...
        if (!Settings->flag.button_single)
        { // SetOption13 (0) - Allow multi-press
            ...
            if (!TasmotaGlobal.restart_flag && !Button.hold_timer[button_index] && (Button.press_counter[button_index] > 0) && (Button.press_counter[button_index] < 7))
            {
              ...
              { // Single or Double press
...
#endif // ESP8266
                {
                  ...
                  if ((1 == Button.present) && (2 == TasmotaGlobal.devices_present))
                  { // Single Button with two devices only
              ...
              if (!RotaryButtonPressed(button_index))
              {
                ...
                else
                {
                  ...
                  { // Single to Penta press
                    if (WifiState() > WIFI_RESTART)
                    { // Wifimanager active
                      TasmotaGlobal.restart_flag = 1;
                    }
                    if (!Settings->flag3.mqtt_buttons)
                    { // SetOption73 - Detach buttons from relays and enable MQTT action state for multipress
                      if (Button.press_counter[button_index] == 1)
                      { // By default first press always send a TOGGLE (2)
                      ...
                      else
                      {
                        ...
                        if (0 == button_index)
                        { // BUTTON1 can toggle up to 5 relays if present. If a relay is not present will send out the key value (2,11,12,13 and 14) for rules
                  ...
                  { // 6 press start wificonfig 2
                    if (!Settings->flag.button_restrict)
                    { // SetOption1  - Control button multipress
